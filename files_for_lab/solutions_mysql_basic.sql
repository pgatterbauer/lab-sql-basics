...
# What is the loan `amount` of the lowest 5 `account_id`s in the `loan` table? Show the `account_id` 
...
#What are the `account_id`s with the lowest loan `amount` that have a loan `duration` of 60 
# in the `loan` table?
...
# In the `order` table, which `account_id`s were responsible for orders between `order_id` 29540 
...
FROM 
	loan
WHERE 
	loan.date BETWEEN 971201 AND 971231
GROUP BY 
	loan.date, duration 
ORDER BY 
	loan.date ASC;
